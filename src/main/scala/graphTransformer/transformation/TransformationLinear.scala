...
trait TransformationLinear[F[_], I, O] extends Transformation[F, I, O] {
  def apply(input: I): F[O]
}

trait TransformationLinearPure[I, O] extends TransformationLinear[UIO, I, O] {
  def apply(input: I): UIO[O] = ZIO.succeed(pureApply(input))

  def pureApply(input: I): O
}

trait TransformationLinearIO[I, O] extends TransformationLinear[Task, I, O] {
