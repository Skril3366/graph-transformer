...
class InitTransformation[T](value: T)
    extends TransformationLinearPure[Unit, T]((), value) {
...
class ToListTransformation[T](element_example: T)
    extends TransformationLinearPure[T, List[T]](
      element_example,
      List(element_example)
    ) {
  override def name: String = "ToListTransformation"
  override def pureApply(input: T) = List(input)
}

class ConcatListsTransformation[T](element_example: T)
    extends TransformationPolyPure[List[T]](List(element_example)) {
